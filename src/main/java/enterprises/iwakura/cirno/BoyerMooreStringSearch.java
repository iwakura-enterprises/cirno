 ...
 * <p>
 * Use {@link #of(String)} to pre-compute the lookup tables once and
...
public class BoyerMooreStringSearch {
     ...
     *
     * @return a reusable {@link Pattern}
     */
    public static Pattern of(String pattern) {
        pattern = StringUtils.normalize(pattern);
        int m = pattern.length();
        if (m < BM_THRESHOLD) {
            return new Pattern(pattern, null, null, true);
        }
        return new Pattern(pattern, buildBadCharTable(pattern, m), buildGoodSuffixTable(pattern, m), false);
    }
    ...
    public static class Pattern {
        ...
        private final Map<Character, Integer> badChar;
        private final int[] goodSuffix;
        private final boolean useNaive;

        /**
         * Constructs {@link Pattern}
         *
         ...
         */
        protected Pattern(String pattern, Map<Character, Integer> badChar, int[] goodSuffix, boolean useNaive) {
